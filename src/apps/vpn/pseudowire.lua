...
-- of such a forwarder is provided by the apps/vpn/bridge.lua module.
--
...
local cc = require("apps.vpn.control_channel")
local ipc_mib = require("lib.ipc.shmem.mib")

pseudowire = subClass(nil)
pseudowire._name = "Pseudowire"

-- Print a log message to stdout using a simple token-bucket for
-- rate-limiting.  The limit applies to all pseudowire instances.
--
-- Maximum number of messages per second.
log_rate = 10

local log_tstamp = C.get_unix_time()
local log_token_bucket = log_rate
local log_discard = 0
local function log(self, msg, print_peer)
   local now = C.get_unix_time()
   local add_tokens = log_rate*(now-log_tstamp)
   log_token_bucket = math.min(log_rate, log_token_bucket+add_tokens)
   if log_token_bucket >= 1 then
      if log_discard > 0 then
	 print(os.date("%b %Y %H:%M:%S ").."discarded "
	    ..log_discard.." log messages")
	 log_discard = 0
      end
      if print_peer then
	 msg = "Peer "..self._transport.peer..": "..msg
      end
      msg = self._name.." ("..self._conf.name.."): "..msg
      print(os.date("%b %Y %H:%M:%S ")..msg)
      log_token_bucket = log_token_bucket-1
   else
      log_discard = log_discard+1
   end
   log_tstamp = now
end
...
function pseudowire:new (arg)
   ...
   o._tunnel = require("apps.vpn.tunnels."..
		       conf.tunnel.type):new(conf.tunnel, conf.cc)

   -- Transport header
   assert(conf.transport, "missing transport configuration")
   o._transport = require("apps.vpn.transports."..
   ...
   local mib = ipc_mib:new({ directory = '/tmp/snabb-shmem',
			     filename = conf.name })
...
function pseudowire:push()
   ...
   while not full(l_out) and not empty(l_in) do
	 ...
	 return
      ...
      local datagram = self._dgram:reuse(p[0], ethernet)
      ...
      datagram:push(self._tunnel.header)
      datagram:push(self._transport.header)
      datagram:push(self._ether)
      transmit(l_out, p[0])
   end
...
local function selftest_aux(type, pseudowire_config)
   local c = config.new()
   local pcap_base = "apps/vpn/pseudowire-selftest"
